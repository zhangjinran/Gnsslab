...
#include <string>
#include <gnsslab/NavEphGalileo.hpp>
#include <gnsslab/CoordStruct.h>

using namespace std;

void NavEphGalileo::printData() const {
    cout << "****************************************************************"
         << "************" << endl
         << "Galileo Broadcast Ephemeris Data: " << endl;
    ...
    cout << "GalileoWeek: " << setw(16) << GalileoWeek << endl;
    cout << "SISA: " << setw(16) << SISA << endl;

    cout << "SV_health: " << setw(16) << SV_health << endl;
    cout << "BGD_E5aE1: " << setw(16) << BGD_E5aE1 << endl;
    cout << "BGD_E5bE1: " << setw(16) << BGD_E5bE1 << endl;
...
double NavEphGalileo::svClockBias(const CommonTime &t) const {
    double dtc, elaptc;
    elaptc = t - ctToc;
...
double NavEphGalileo::svClockDrift(const CommonTime &t) const {
    double drift, elaptc;
    elaptc = t - ctToc;
...
long double NavEphGalileo::svRelativity(const CommonTime &t) const {
    Galileo ell;
...
double NavEphGalileo::svURA(const CommonTime &t) const {
    double ephURA = SISA;
    return ephURA;
}

Xvt NavEphGalileo::svXvt(const CommonTime &t) const {
    Xvt sv;
    Galileo ell;
...
bool NavEphGalileo::isValid(const CommonTime &ct) const {
    if (ct.m_timeSystem != TimeSystem::GAL) return false;
