...
#undef tswap
   ...
   inline MatrixXd rotation(double angle, int axis)
      noexcept(false)
   {
      if (axis < 1 || axis > 3)
      {
         InvalidRequest e("Invalid axis (must be 1,2, or 3)");
         throw(e);
      }

      MatrixXd toReturn(3,3);
      toReturn.setZero();
   ...
   inline double normF(const MatrixXd& m)
   {
      double sum(0);
      size_t i,j;
      for (i = 0; i < m.rows(); i++)
   ...
   inline double norm(const VectorXd& v)
   {
      double mag(0);
      ...
      {
         if(mag > ABS(v(i)))
            mag *= SQRT(double(1)+(v(i)/mag)*(v(i)/mag));
         else if(ABS(v(i)) > mag)
            mag = ABS(v(i))*SQRT(double(1)+(mag/v(i))*(mag/v(i)));
         else
            mag *= SQRT(double(2));
      }
      return mag;
