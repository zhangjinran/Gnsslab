#include <string>
#include <iomanip>
#include <gnsslab/NavEphIRNSS.hpp>

using namespace std;

void NavEphIRNSS::printData() const {
    cout << "****************************************************************"
         << "************" << endl
         << "IRNSS Broadcast Ephemeris Data: " << endl;
    ...
    cout << "IRNSSWeek: " << setw(16) << IRNSSWeek << endl;
...
double NavEphIRNSS::svClockBias(const CommonTime &t) const {
    double dtc, elaptc;
    elaptc = t - ctToc;
...
double NavEphIRNSS::svClockDrift(const CommonTime &t) const {
    double drift, elaptc;
    elaptc = t - ctToc;
...
long double NavEphIRNSS::svRelativity(const CommonTime &t) const {
    GPSEllipsoid ell;
...
double NavEphIRNSS::svURA(const CommonTime &t) const {
    double ephURA = URA;
    return ephURA;
}

Xvt NavEphIRNSS::svXvt(const CommonTime &t) const {
    Xvt sv;
    GPSEllipsoid ell;
...
bool NavEphIRNSS::isValid(const CommonTime &ct) const {
    if (ct.m_timeSystem != TimeSystem::IRN) return false;
